...
fig, ax = plt.subplots()
plt.subplots_adjust(left=0,bottom=0.28,right=0.74,top=1)
...
def IRB1600v(t1,t2,t3,t4,t5,t6):
    t0=np.eye(4)
    t01=rotaz(t1)#
    t12=trasz(486.5)@rotax(-90)#
    t23=trasx(150)@rotaz(t2)
...
def IRB1600newton(vd,sem):
    ...
    while calc:
        ...
        n=0
        while n<6:
            td=IRB1600v(tv1[n],tv2[n],tv3[n],tv4[n],tv5[n],tv6[n])
            tj=(td-t)/d
            j[0,nc]=tj[0,0]
            j[1,nc]=tj[0,1]
            j[2,nc]=tj[0,2]
...
#RDK=Robolink()
#robot=RDK.Item('IRB1600')

axn1=plt.axes([0.1,0.2,0.3,0.03])
axn2=plt.axes([0.1,0.15,0.3,0.03])
axn3=plt.axes([0.1,0.1,0.3,0.03])
axn4=plt.axes([0.55,0.2,0.3,0.03])
axn5=plt.axes([0.55,0.15,0.3,0.03])
axn6=plt.axes([0.55,0.1,0.3,0.03])
axn7=plt.axes([1.3,0.5,0.07,0.03])

sn1=Slider(axn1, 'rot 1', -180, 180.0, valinit=0)
sn2=Slider(axn2, 'rot 2', -90, 150.0, valinit=0)
sn3=Slider(axn3, 'rot 3', -238, 79.0, valinit=0)
sn4=Slider(axn4, 'rot 4', -155, 155.0, valinit=0)
sn5=Slider(axn5, 'rot 5', -90, 135.0, valinit=0)
sn6=Slider(axn6, 'rot 6', -200, 200.0, valinit=0)
...
def update(val):
    ...
    while x<4:
        while y<4:
            tabla._cells[(x,y)]._text.set_text(np.round(MTH[x,y],3))
            y=y+1
        y=0
        x=x+1
    dibujar()
...
ax.cla()
setaxis(1000)
MTH=IRB1600(0,0,0,0,0,0)
tabla=plt.table(cellText=np.round(MTH,3),bbox=[-9.35,0,5,5])
